...
public class AccountController {
	...
	public Map<String, Object> settingInfo(
			@RequestParam("user_name") String user_name,
			@RequestParam("user_desc") String user_desc, HttpSession session) {
	...
	public ModelAndView resetpwdPage(@RequestParam("key") String key,
		...
		mav.addObject("SUCCESS_PWD_RESET_ALLOWED",
				Property.SUCCESS_PWD_RESET_ALLOWED);
		mav.addObject("ERROR_PWD_RESET_NOTALLOWED",
				Property.ERROR_PWD_RESET_NOTALLOWED);
	...
	public Map<String, Object> resetpwd(
		...
		map.put("status", userService.resetPassword(user.getUser_email(),
				password, cfm_pwd));
		return map;
	}
	...
	public Map<String, Object> changepwd(
			...
			@RequestParam("new_pwd") String new_pwd, HttpSession session) {
		Map<String, Object> map = new HashMap<String, Object>();
		User user = (User) session.getAttribute("user");
		map.put("status", userService.changePassword(user.getUser_email(),
				old_pwd, new_pwd));
		return map;
	}
	...
	public Map<String, Object> sendResetPwdEmail(HttpSession session) {
		Map<String, Object> map = new HashMap<String, Object>();
		User user = (User) session.getAttribute("user");
		mailService.sendResetPwdEmail(user.getUser_email(),
				userService.updateResetPwdKey(user.getUser_email()));
	...
	public Map<String, Object> login(@RequestParam("email") String email,
		 ...
		 * if(Property.SUCCESS_ACCOUNT_LOGIN.equals(status)) { User user =
		 * userService.findByEmail(email); session.setAttribute("user", user); }
		 */
	...
	public Map<String, String> acticationMailResend(
			@RequestParam("email") String email) {
